...
%   
    pr = obj.layers(l).paramIndexes;
    if ~isempty(pr) 
        normmax = 1000;
        maxapara = max(abs(obj.params(pr(1)).der(:)));
%         if maxapara>normmax
            a = normmax/maxapara;
            obj.params(pr(1)).der = obj.params(pr(1)).der * a;
%         end
        if numel(pr)>1
            maxbpara = max(abs(obj.params(pr(2)).der(:)));
%             if maxbpara>normmax
                b = normmax/maxbpara;
                obj.params(pr(2)).der = obj.params(pr(2)).der * b;
%             end
        end
    end
